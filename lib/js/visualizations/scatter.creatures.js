function paint() {
  ...
  var svg = d3.select("#scatter").append("svg").attr("width", width + margin.left + margin.right).attr("height", height + margin.top + margin.bottom).append("g").attr("transform", "translate(" + margin.left + "," + margin.top + ")");
  d3.csv('../../assets/data/creatures.csv', function (error, data) {
    ...
    var tipMouseover = function (d) {
      ...
      color = '#8DACC6';
      color2 = 'black';
      var html = "<span class='tooltip-dimensions' style='color:" + color + ";'>" + "<b style='color:" + color + ";'>" + d.Creature + "</b></span><br>" + "<span style='color:" + color2 + ";'><b style='color:" + color2 + ";'>" + d.Mass + "</b> Grams, <b style='color:" + color2 + ";'>" + d.RestingHeartRate + "</b> bpm, <b style='color:" + color2 + ";'>" + d.Longevity + "</b> Years</span>";
    ...
    }).attr("r", 5).style("fill", "#8DACC6").on("mouseover", tipMouseover).on("mouseout", tipMouseout);
  });
}
